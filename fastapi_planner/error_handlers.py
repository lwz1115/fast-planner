...
async def depth_processing_exception_handler(
    request: Request,
    exc: DepthProcessingException
) -> JSONResponse:
    """
    Handle depth processing errors (400).
    
    Handles:
    - Invalid depth image format
    - Dimension mismatch between image and camera parameters
    - Invalid depth encoding
    - Other depth processing validation errors
    
    Args:
        request: The FastAPI request
        exc: The depth processing exception
    ...
    """
    logger.error(
        f"Depth processing error: {exc.code} - {exc.message}",
        extra={
            "error_code": exc.code,
            "path": request.url.path,
            "details": exc.details
...
async def ros_publish_failed_exception_handler(
    request: Request,
    exc: ROSPublishFailedException
) -> JSONResponse:
    """
    Handle ROS publishing failures (503).
    
    Handles failures when publishing point cloud data to ROS topics.
    This typically indicates ROS connection issues or topic unavailability.
    
    Args:
        request: The FastAPI request
        exc: The ROS publish failed exception
    ...
    """
    logger.error(
        f"ROS publish failed: {exc.code} - {exc.message}",
        extra={
            "error_code": exc.code,
            "path": request.url.path,
            "details": exc.details
...
def register_exception_handlers(app) -> None:
    ...
    Requirements: 3.1, 3.2, 3.3, 3.4, 3.5, 12.4, 12.5
    """
    # Register depth processing exception handlers (must be before ValidationException)
