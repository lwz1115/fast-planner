...
void SDFMap::initMap(rclcpp::Node& nh) {
  node_ = nh;

  /* get parameter */
  double x_size, y_size, z_size;
  mp_.resolution_ = node_->declare_parameter("sdf_map/resolution", -1.0);
  x_size = node_->declare_parameter("sdf_map/map_size_x", -1.0);
  y_size = node_->declare_parameter("sdf_map/map_size_y", -1.0);
  z_size = node_->declare_parameter("sdf_map/map_size_z", -1.0);
  mp_.local_update_range_(0) = node_->declare_parameter("sdf_map/local_update_range_x", -1.0);
  mp_.local_update_range_(1) = node_->declare_parameter("sdf_map/local_update_range_y", -1.0);
  mp_.local_update_range_(2) = node_->declare_parameter("sdf_map/local_update_range_z", -1.0);
  mp_.obstacles_inflation_ = node_->declare_parameter("sdf_map/obstacles_inflation", -1.0);

  mp_.fx_ = node_->declare_parameter("sdf_map/fx", -1.0);
  mp_.fy_ = node_->declare_parameter("sdf_map/fy", -1.0);
  mp_.cx_ = node_->declare_parameter("sdf_map/cx", -1.0);
  mp_.cy_ = node_->declare_parameter("sdf_map/cy", -1.0);

  mp_.use_depth_filter_ = node_->declare_parameter("sdf_map/use_depth_filter", true);
  mp_.depth_filter_tolerance_ = node_->declare_parameter("sdf_map/depth_filter_tolerance", -1.0);
  mp_.depth_filter_maxdist_ = node_->declare_parameter("sdf_map/depth_filter_maxdist", -1.0);
  mp_.depth_filter_mindist_ = node_->declare_parameter("sdf_map/depth_filter_mindist", -1.0);
  mp_.depth_filter_margin_ = node_->declare_parameter("sdf_map/depth_filter_margin", -1);
  mp_.k_depth_scaling_factor_ = node_->declare_parameter("sdf_map/k_depth_scaling_factor", -1.0);
  mp_.skip_pixel_ = node_->declare_parameter("sdf_map/skip_pixel", -1);

  mp_.p_hit_ = node_->declare_parameter("sdf_map/p_hit", 0.70);
  mp_.p_miss_ = node_->declare_parameter("sdf_map/p_miss", 0.35);
  mp_.p_min_ = node_->declare_parameter("sdf_map/p_min", 0.12);
  mp_.p_max_ = node_->declare_parameter("sdf_map/p_max", 0.97);
  mp_.p_occ_ = node_->declare_parameter("sdf_map/p_occ", 0.80);
  mp_.min_ray_length_ = node_->declare_parameter("sdf_map/min_ray_length", -0.1);
  mp_.max_ray_length_ = node_->declare_parameter("sdf_map/max_ray_length", -0.1);

  mp_.esdf_slice_height_ = node_->declare_parameter("sdf_map/esdf_slice_height", -0.1);
  mp_.visualization_truncate_height_ = node_->declare_parameter("sdf_map/visualization_truncate_height", -0.1);
  mp_.virtual_ceil_height_ = node_->declare_parameter("sdf_map/virtual_ceil_height", -0.1);

  mp_.show_occ_time_ = node_->declare_parameter("sdf_map/show_occ_time", false);
  mp_.show_esdf_time_ = node_->declare_parameter("sdf_map/show_esdf_time", false);
  mp_.pose_type_ = node_->declare_parameter("sdf_map/pose_type", 1);

  mp_.frame_id_ = node_->declare_parameter("sdf_map/frame_id", string("world"));
  mp_.local_bound_inflate_ = node_->declare_parameter("sdf_map/local_bound_inflate", 1.0);
  mp_.local_map_margin_ = node_->declare_parameter("sdf_map/local_map_margin", 1);
  mp_.ground_height_ = node_->declare_parameter("sdf_map/ground_height", 1.0);
  ...
  indep_cloud_sub_ =
      /* TODO: 转换订阅 */ node_->create_subscription<sensor_msgs::msg::PointCloud2>("/sdf_map/cloud", 10, &SDFMap::cloudCallback, this);
  indep_odom_sub_ =
      /* TODO: 转换订阅 */ node_->create_subscription<nav_msgs::msg::Odometry>("/sdf_map/odom", 10, &SDFMap::odomCallback, this);

  occ_timer_ = node_->create_wall_timer(std::chrono::duration<double>(0.05), &SDFMap::updateOccupancyCallback, this);
  esdf_timer_ = node_->create_wall_timer(std::chrono::duration<double>(0.05), &SDFMap::updateESDFCallback, this);
  vis_timer_ = node_->create_wall_timer(std::chrono::duration<double>(0.05), &SDFMap::visCallback, this);

  map_pub_ = /* TODO: 转换发布 */ node_->create_publisher<sensor_msgs::msg::PointCloud2>("/sdf_map/occupancy", 10);
  map_inf_pub_ = /* TODO: 转换发布 */ node_->create_publisher<sensor_msgs::msg::PointCloud2>("/sdf_map/occupancy_inflate", 10);
  esdf_pub_ = /* TODO: 转换发布 */ node_->create_publisher<sensor_msgs::msg::PointCloud2>("/sdf_map/esdf", 10);
  update_range_pub_ = /* TODO: 转换发布 */ node_->create_publisher<visualization_msgs::msg::Marker>("/sdf_map/update_range", 10);

  unknown_pub_ = /* TODO: 转换发布 */ node_->create_publisher<sensor_msgs::msg::PointCloud2>("/sdf_map/unknown", 10);
  depth_pub_ = /* TODO: 转换发布 */ node_->create_publisher<sensor_msgs::msg::PointCloud2>("/sdf_map/depth_cloud", 10);
    ...
    for (int y = min_esdf(1); y <= max_esdf(1); ++y)
      for (int z = min_esdf(2); z <= max_esdf(2); ++z) {
        ...
        if (md_.occupancy_buffer_inflate_[idx] == 0) {
          ...
          RCLCPP_ERROR(node_->get_logger(), this->get_logger(), "what?");
        }
      }
...
void SDFMap::updateOccupancyCallback(const rclcpp::TimerEvent& /*event*/) {
  ...
  if (mp_.show_occ_time_)
    RCLCPP_WARN(node_->get_logger(), this->get_logger(), "Fusion: cur t = %lf, avg t = %lf, max t = %lf", (t2 - t1).seconds(),
...
void SDFMap::updateESDFCallback(const rclcpp::TimerEvent& /*event*/) {
  ...
  if (mp_.show_esdf_time_)
    RCLCPP_WARN(node_->get_logger(), this->get_logger(), "ESDF: cur t = %lf, avg t = %lf, max t = %lf", (t2 - t1).seconds(),
             md_.esdf_time_ / md_.update_num_, md_.max_esdf_time_);

  md_.esdf_need_update_ = false;
}

void SDFMap::depthPoseCallback(const sensor_msgs::msg::ImageSharedPtr& img,
                               const geometry_msgs::msg::PoseStampedSharedPtr& pose) {
  /* get depth image */
  cv_bridge::CvImagePtr cv_ptr;
  cv_ptr = cv_bridge::toCvCopy(img, img->encoding);

  if (img->encoding == sensor_msgs::msg::image_encodings::TYPE_32FC1) {
...
void SDFMap::odomCallback(const nav_msgs::msg::OdometrySharedPtr& odom) {
...
void SDFMap::cloudCallback(const sensor_msgs::msg::PointCloud2SharedPtr& img) {
...
void SDFMap::publishMap() {
  ...
  // RCLCPP_INFO(node_->get_logger(), this->get_logger(), "pub map");
...
void SDFMap::publishESDF() {
  ...
  // RCLCPP_INFO(node_->get_logger(), this->get_logger(), "pub esdf");
}
...
void SDFMap::depthOdomCallback(const sensor_msgs::msg::ImageSharedPtr& img,
                               const nav_msgs::msg::OdometrySharedPtr& odom) {
  /* get pose */
  md_.camera_pos_(0) = odom->pose.pose.position.x;
  md_.camera_pos_(1) = odom->pose.pose.position.y;
  md_.camera_pos_(2) = odom->pose.pose.position.z;
...
void SDFMap::depthCallback(const sensor_msgs::msg::ImageSharedPtr& img) {
  std::cout << "depth: " << img->header.stamp << std::endl;
}

void SDFMap::poseCallback(const geometry_msgs::msg::PoseStampedSharedPtr& pose) {
